...
// ============================================
(function() {
  WMS.renderUsers = function(container) {
    var users = WMS.Users.getAll(), cur = WMS.getCurrentUser(), canWrite = WMS.hasPermission('users', 'write');
    ...
    if (ab) ab.addEventListener('click', function() { openUModal(); });
    document.getElementById('saveUserBtn').addEventListener('click', function() { saveU(container); });
    container.querySelectorAll('.edit-user-btn').forEach(function(b) { b.addEventListener('click', function() { openUModal(b.dataset.id); }); });
    container.querySelectorAll('.delete-user-btn').forEach(function(b) { b.addEventListener('click', function() { delU(b.dataset.id, container); }); });
  };

  function openUModal(editId) {
    document.getElementById('userForm').reset(); document.getElementById('userEditId').value = '';
    if (editId) {
      var u = WMS.Users.getById(editId); if(!u) return;
  ...
  function saveU(container) {
    ...
    if (editId) {
      ...
      WMS.Users.update(editId, upd); WMS.showToast('Usuario actualizado.', 'success');
    } else {
      if (!pw || pw.length < 8) { WMS.showToast('Contraseña mínimo 8 caracteres.', 'warning'); return; }
      if (WMS.Users.getByEmail(email)) { WMS.showToast('Email ya registrado.', 'error'); return; }
      WMS.Users.create({ name:name, email:email, password:pw, role:document.getElementById('userRole').value, status:document.getElementById('userStatus').value });
      WMS.showToast('Usuario creado.', 'success');
    }
    WMS.closeModal('userModal'); WMS.renderUsers(container);
  }

  function delU(id, container) {
    var u = WMS.Users.getById(id);
    if (!u || !confirm('¿Eliminar "' + u.name + '"?')) return;
    var r = WMS.Users.delete(id);
    if (r && r.error) { WMS.showToast(r.error, 'error'); return; }
    WMS.showToast('Usuario eliminado.', 'success'); WMS.renderUsers(container);
  }
})();
