...
(function() {
  WMS.renderDashboard = async function(container) {
    ...
    container.innerHTML = '<div class="page-header"><div><h1 class="page-title">Dashboard</h1><p class="page-subtitle">Bienvenido, ' + (user?user.name:'Usuario') + '. Resumen operativo del almacén.</p></div></div>'
      ...
      + '<div class="kpi-card animate-fade-in" style="animation-delay:400ms"><div class="kpi-card-icon blue">🔄</div><div class="kpi-card-value">' + WMS.formatNumber(mt) + '</div><div class="kpi-card-label">Movimientos Hoy</div><div style="font-size:var(--font-xs);color:var(--text-muted);margin-top:var(--space-1)">' + WMS.formatNumber(mm) + ' este mes</div></div>'
      + '</div>'
