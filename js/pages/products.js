...
(function() {
  ...
  async function renderPage(container) {
    ...
    var rows = ''; pg.data.forEach(function(p) {
      ...
      rows += '<tr><td><span class="product-sku">' + p.sku + '</span></td><td>' + p.description + '</td><td><span class="badge badge-neutral">' + (p.category||'—') + '</span></td><td>' + (p.unit||'uds') + '</td><td><span class="badge ' + (isLow?'badge-danger':'badge-success') + '">' + WMS.formatNumber(st) + '</span></td><td>' + (p.minStock?WMS.formatNumber(p.minStock):'—') + '</td><td>' + (p.inventario||'0') + '</td><td>' + (p.lotRequired?'<span class="badge badge-warning">Sí</span>':'No') + '</td>'
    ...
    container.innerHTML = '<div class="page-header"><div><h1 class="page-title">Productos</h1><p class="page-subtitle">Catálogo de productos del almacén</p></div><div style="display:flex;gap:var(--space-2)">' + (canWrite?'<button class="btn btn-secondary" id="importExcelBtn">📄 Cargar Excel</button><input type="file" id="excelInput" style="display:none" accept=".xlsx, .xls"><button class="btn btn-primary" id="addProductBtn">+ Nuevo Producto</button>':'') + '</div></div>'
      ...
      '<div class="table-wrapper"><table class="table"><thead><tr><th>Código</th><th>Descripción</th><th>Categoría</th><th>Unidad</th><th>Stock</th><th>Stock Mín.</th><th>Inventario</th><th>Lote Req.</th>' + (canWrite?'<th>Acciones</th>':'') + '</tr></thead><tbody>' + rows + '</tbody></table></div><div class="pagination" id="productPagination"></div>')
      + '</div>'
