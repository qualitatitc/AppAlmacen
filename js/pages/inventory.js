...
(function() {
  ...
  WMS.renderInventory = function(container, sub) {
    ...
    container.querySelectorAll('.tab').forEach(function(tab) {
      tab.addEventListener('click', function() {
        ...
        renderTab(document.getElementById('inventoryContent'));
      });
    });
    renderTab(document.getElementById('inventoryContent'));
  };
  ...
  function renderTab(el) {
    if (activeTab === 'entry') renderEntry(el);
    else if (activeTab === 'exit') renderExit(el);
    else if (activeTab === 'stock') renderStock(el);
    else if (activeTab === 'movements') renderMovements(el);
  }
  ...
  function productOptions() {
    return WMS.Products.getAll().map(function(p) { return '<option value="' + p.id + '">' + p.sku + '</option>'; }).join('');
  }
  ...
  function renderEntry(el) {
    el.innerHTML = '<div class="card animate-fade-in"><div class="card-header"><h3 class="card-title">📥 Registrar Entrada de Mercancía</h3><span class="badge badge-success">Nueva entrada</span></div>'
      ...
      + '<div style="display:flex;gap:var(--space-3);justify-content:flex-end"><button type="reset" class="btn btn-secondary">Limpiar</button><button type="submit" class="btn btn-success">📥 Registrar Entrada</button></div></form></div>';
    ...
    document.getElementById('entryForm').addEventListener('submit', function(e) {
      ...
      var prod = WMS.Products.getById(pid);
      if (prod && prod.lotRequired && !lot) { WMS.showToast('Este producto requiere número de lote.', 'warning'); return; }
      WMS.Inventory.addStock(pid, lid, qty, lot, null, '', row, pos);
      var preciseLoc = lid; if (row||pos) preciseLoc += '|' + row + '|' + pos;
      WMS.Movements.create({ type:'entrada', productId:pid, quantity:qty, locationTo:preciseLoc, lot:lot, supplier:document.getElementById('entrySupplier').value.trim(), reference:document.getElementById('entryRef').value.trim(), notes:document.getElementById('entryNotes').value.trim(), userId:(WMS.getCurrentUser()||{}).id, entryNumber:'ENT-'+Date.now().toString(36).toUpperCase() });
  ...
  function renderExit(el) {
    el.innerHTML = '<div class="card animate-fade-in"><div class="card-header"><h3 class="card-title">📤 Registrar Salida de Mercancía</h3><span class="badge badge-danger">Nueva salida</span></div>'
      ...
      + '<div style="display:flex;gap:var(--space-3);justify-content:flex-end"><button type="reset" class="btn btn-secondary">Limpiar</button><button type="submit" class="btn btn-danger">📤 Registrar Salida</button></div></form></div>';
    ...
    var updateInfo = function() {
      ...
      if (!pid) return;
      var entries = WMS.Inventory.getAll().filter(function(i) { return i.productId===pid && i.quantity>0; });
      entries.forEach(function(en) {
        var loc = WMS.Locations.getAll().find(function(l){return l.id===en.locationId;});
    ...
    document.getElementById('exitForm').addEventListener('submit', function(e) {
      ...
      var r = WMS.Inventory.removeStock(pid, lid, qty, lot, '', row, pos);
      if (r && r.error) { WMS.showToast(r.error, 'error'); return; }
      var prod = WMS.Products.getById(pid);
      var preciseLoc = lid; if (row||pos) preciseLoc += '|' + row + '|' + pos;
      WMS.Movements.create({ type:'salida', productId:pid, quantity:qty, locationFrom:preciseLoc, reason:'venta', lot:lot, reference:document.getElementById('exitRef').value.trim(), notes:document.getElementById('exitNotes').value.trim(), userId:(WMS.getCurrentUser()||{}).id });
  ...
  function renderStock(el) {
    var items = WMS.Inventory.getStockSummary();
    ...
    var pg = WMS.paginate(items, stockPage), locs = WMS.Locations.getAll();
    ...
    if (si) si.addEventListener('input', WMS.debounce(function(e) { stockSearch = e.target.value; stockPage = 1; renderStock(el); }, 250));
    var pEl = document.getElementById('stockPagination');
    if (pEl) WMS.renderPagination(pEl, pg, function(p) { stockPage = p; renderStock(el); });
    el.querySelectorAll('.stock-row').forEach(function(row) {
      row.addEventListener('click', function() {
        var pid = row.dataset.productId, prod = WMS.Products.getById(pid); if(!prod) return;
        var entries = WMS.Inventory.getAll().filter(function(i) { return i.productId===pid && i.quantity>0; });
        var recMoves = WMS.Movements.getByProduct(pid).sort(function(a,b){return new Date(b.timestamp)-new Date(a.timestamp);}).slice(0,5);
        var deHtml = '<div class="card animate-fade-in"><div class="card-header"><h3 class="card-title">📦 ' + prod.sku + ' — ' + prod.description + '</h3><span class="badge badge-primary">Stock: ' + WMS.formatNumber(WMS.Inventory.getTotalStock(pid)) + '</span></div><h4 style="color:var(--text-secondary);font-size:var(--font-sm);margin-bottom:var(--space-3)">Distribución por Ubicación</h4>';
        if (entries.length === 0) deHtml += '<p class="text-muted" style="font-size:var(--font-sm)">Sin stock</p>';
          ...
          deHtml += '<div class="stock-location-item"><div><span class="stock-location-code">' + (loc?loc.code:e.locationId) + '</span><span class="text-muted" style="font-size:var(--font-xs);margin-left:var(--space-2)">' + (loc?loc.name:'') + '</span>' + (e.lot?'<span class="badge badge-info" style="margin-left:var(--space-2)">Lote: '+e.lot+'</span>':'') + '</div><div><span class="stock-location-qty">' + WMS.formatNumber(e.quantity) + '</span></div></div>';
        });
        if (recMoves.length > 0) {
          var formatLoc = function(val) {
            ...
            var d = (l?l.code:p[0]);
            if (p[1]!=='') d += '-F' + (parseInt(p[1]) + 1); // Row is 0-indexed in DB, but F1, F2... for user
            if (p[2]!=='') d += '-P' + p[2];
            return d;
          };
  ...
  function renderMovements(el) {
    var items = WMS.Movements.getAll(), prods = WMS.Products.getAll(), locs = WMS.Locations.getAll();
    ...
    if (mf) mf.addEventListener('change', function(e) { movFilter = e.target.value; movPage = 1; renderMovements(el); });
    var pEl = document.getElementById('movPagination');
    if (pEl) WMS.renderPagination(pEl, pg, function(p) { movPage = p; renderMovements(el); });
  }
})();
